...
params:
  ...
  xlsx_path: "Master_Statistics_NonParametric.xlsx"
  exclude_ids: !r c()
...
# Read Sheets (Non-Parametric names)
wc_cong <- tryCatch(read.xlsx(xlsx_file, sheet = "Wilcoxon_Congruency"), error = function(e) NULL)
fried_img <- tryCatch(read.xlsx(xlsx_file, sheet = "Friedman_Image"), error = function(e) NULL)
wc_img_int <- tryCatch(read.xlsx(xlsx_file, sheet = "Wilcoxon_Image_Interaction"), error = function(e) NULL)
fried_txt <- tryCatch(read.xlsx(xlsx_file, sheet = "Friedman_Text"), error = function(e) NULL)
wc_txt_val <- tryCatch(read.xlsx(xlsx_file, sheet = "Wilcoxon_Text_Valence_Int"), error = function(e) NULL)
wc_txt_aro <- tryCatch(read.xlsx(xlsx_file, sheet = "Wilcoxon_Text_Arousal_Int"), error = function(e) NULL)
wc_txt_3way <- tryCatch(read.xlsx(xlsx_file, sheet = "Wilcoxon_Text_3Way_Int"), error = function(e) NULL)

# Permutation Results
perm_cong <- tryCatch(read.xlsx(xlsx_file, sheet = "Permutation_Congruency"), error = function(e) NULL)
perm_img_int <- tryCatch(read.xlsx(xlsx_file, sheet = "Permutation_Image_Interaction"), error = function(e) NULL)
perm_txt_val <- tryCatch(read.xlsx(xlsx_file, sheet = "Permutation_Text_Valence"), error = function(e) NULL)
perm_txt_aro <- tryCatch(read.xlsx(xlsx_file, sheet = "Permutation_Text_Arousal"), error = function(e) NULL)
perm_txt_3way <- tryCatch(read.xlsx(xlsx_file, sheet = "Permutation_Text_3Way"), error = function(e) NULL)
...
get_perm_text <- function(perm_res) {
  if(is.null(perm_res)) return("")
  p <- as.numeric(perm_res$P_Value)
  stat <- round(as.numeric(perm_res$Statistic), 3)
  sig_txt <- get_sig_text(p)
  cls <- ifelse(p < 0.05, "sig", "nonsig")
  
  return(paste0(
    "<div style='margin-top:10px; padding:10px; background:#f0f8ff; border-radius:5px;'>",
    "<strong>置換檢定 (Permutation Test) 結果：</strong><br>",
    "檢定結果：<span class='", cls, "'>", sig_txt, "</span><br>",
    "統計量 Z = ", stat, ", p = ", format.pval(p, digits=3),
    "</div>"
  ))
}
...
# `r report_title` - 無母數統計結果解讀

本報告採用無母數方法（Wilcoxon / Friedman）對行為數據進行驗證。
...
## 1. 整體一致性效應 (Wilcoxon Signed-Rank)

<div class="interpretation-card">
<h3 class="card-title">配對樣本 Wilcoxon 檢定</h3>
```{r cong_interp, results='asis'}
if(!is.null(wc_cong)) {
  p_val <- as.numeric(wc_cong$p)
  stat <- wc_cong$statistic
  n_obs <- if ("n1.y" %in% names(wc_cong)) wc_cong$n1.y else NA  # 每條件的配對樣本數（n1 = n2）
  r_eff <- wc_cong$effsize
  
  if (is.na(n_obs) && !is.null(beh_trials) && nrow(beh_trials) > 0) {
    subj_m <- beh_trials %>%
      filter(acc_used == 1, !is.na(rt), condition %in% c("Congruent", "Incongruent")) %>%
      group_by(subject, condition) %>%
      summarise(mean_rt = mean(rt, na.rm = TRUE), .groups = "drop") %>%
      tidyr::pivot_wider(names_from = condition, values_from = mean_rt)
    n_obs <- sum(!is.na(subj_m$Congruent) & !is.na(subj_m$Incongruent))
  }
  ...
  cat(paste0("<div>統計量：V = ", stat, ", p = ", format.pval(p_val, digits=3), ", N = ", n_obs, "</div>"))
  cat(paste0("<div>效果量 (r)：", round(r_eff, 3), "</div>"))
  
  if(p_val < 0.05) {
    cat("<p class='mt-3'><strong>推論：</strong> 無母數檢定證實，不一致情境的反應時間顯著異於一致情境。</p>")
  } else {
    cat("<p class='mt-3'><strong>推論：</strong> 未觀察到顯著的整體一致性效應。</p>")
  }
  
  cat(get_perm_text(perm_cong))
...
## 2. 圖片實驗：交互作用 (Wilcoxon on Differences)

<div class="interpretation-card">
<h3 class="card-title">圖片類型 × 一致性</h3>
```{r img_int, results='asis'}
if(!is.null(wc_img_int)) {
  p_int <- as.numeric(wc_img_int$p)
  stat <- wc_img_int$statistic
  n_obs <- if ("n1.y" %in% names(wc_img_int)) wc_img_int$n1.y else NA
  r_eff <- wc_img_int$effsize
  sig_class <- ifelse(p_int < 0.05, "sig", "nonsig")
  
  if (is.na(n_obs) && !is.null(beh_trials) && nrow(beh_trials) > 0) {
    eff_by_type <- beh_trials %>%
      ...
      tidyr::pivot_wider(names_from = condition, values_from = mean_rt) %>%
      mutate(effect = Incongruent - Congruent) %>%
      select(subject, Image_Type, effect) %>%
      tidyr::pivot_wider(names_from = Image_Type, values_from = effect)
    # 有兩種圖片類型才算有效
    n_obs <- eff_by_type %>%
      transmute(ok = rowSums(!is.na(.)) >= 2) %>%
      summarise(n = sum(ok)) %>%
      pull(n)
  }
    
  cat(paste0("<div class='result-line'>交互作用 (比較一致性效應差值)：<span class='", sig_class, "'>", get_sig_text(p_int), "</span></div>"))
  cat(paste0("<div>統計量：V = ", stat, ", p = ", format.pval(p_int, digits=3),
             ifelse(!is.na(n_obs), paste0(", N = ", n_obs), ""),
             "</div>"))
    
  if(p_int < 0.05) {
    cat("<p class='mt-3'><strong>推論：</strong> 圖片類型顯著調節了一致性效應的大小 (Interation Significant)。</p>")
  } else {
    cat("<p class='mt-3'><strong>推論：</strong> 未發現顯著交互作用。</p>")
  }
  
  cat(get_perm_text(perm_img_int))
...
if (!is.null(beh_trials) && nrow(beh_trials) > 0) {
  ...
  if (nrow(eff_long) > 0) {
    p <- plotly::plot_ly(
      ...
      mode = "lines+markers",
      hovertemplate = paste0(
        "Subject=%{color}<br>Image_Type=%{x}<br>CE=%{y:.3f}s<extra></extra>"
      )
    ) %>%
      plotly::layout(
        title = "Corresponding Plot: Image Interaction (CE by Image_Type; CE = Incongruent - Congruent)",
        xaxis = list(title = "Image_Type"),
        yaxis = list(title = "Congruency Effect (s)")
      )
    p
  }
}
...
<div class="interpretation-card">
<h3 class="card-title">Valence × Condition</h3>
```{r txt_val, results='asis'}
if(!is.null(wc_txt_val)) {
  p_int <- as.numeric(wc_txt_val$p)
  stat <- wc_txt_val$statistic
  n_obs <- if ("n1.y" %in% names(wc_txt_val)) wc_txt_val$n1.y else NA
  sig_class <- ifelse(p_int < 0.05, "sig", "nonsig")
  
  if (is.na(n_obs) && !is.null(beh_trials) && nrow(beh_trials) > 0) {
    eff_by_val <- beh_trials %>%
      filter(Modality == "Text", acc_used == 1, !is.na(rt), !is.na(Valence),
             condition %in% c("Congruent", "Incongruent")) %>%
      group_by(subject, Valence, condition) %>%
      summarise(mean_rt = mean(rt, na.rm = TRUE), .groups = "drop") %>%
      tidyr::pivot_wider(names_from = condition, values_from = mean_rt) %>%
      mutate(effect = Incongruent - Congruent) %>%
      select(subject, Valence, effect) %>%
      tidyr::pivot_wider(names_from = Valence, values_from = effect)
    n_obs <- eff_by_val %>%
      transmute(ok = rowSums(!is.na(.)) >= 2) %>%
      summarise(n = sum(ok)) %>%
      pull(n)
  ...
  cat(paste0("<div class='result-line'>交互作用 (比較 CE)：<span class='", sig_class, "'>", get_sig_text(p_int), "</span></div>"))
  cat(paste0("<div>統計量：V = ", stat, ", p = ", format.pval(p_int, digits=3),
             ifelse(!is.na(n_obs), paste0(", N = ", n_obs), ""),
             "</div>"))
    
  if(p_int < 0.05) {
    cat("<p class='mt-3'><strong>推論：</strong> 情緒效價顯著調節了一致性效應的大小。</p>")
  } else {
    cat("<p class='mt-3'><strong>推論：</strong> 未發現顯著的 Valence × Condition 交互作用。</p>")
  }
  
  cat(get_perm_text(perm_txt_val))
...
<h3 class="card-title mt-4">Arousal × Condition</h3>
```{r txt_aro, results='asis'}
if(!is.null(wc_txt_aro)) {
  p_int <- as.numeric(wc_txt_aro$p)
  stat <- wc_txt_aro$statistic
  n_obs <- if ("n1.y" %in% names(wc_txt_aro)) wc_txt_aro$n1.y else NA
  sig_class <- ifelse(p_int < 0.05, "sig", "nonsig")
  
  if (is.na(n_obs) && !is.null(beh_trials) && nrow(beh_trials) > 0) {
    eff_by_aro <- beh_trials %>%
      filter(Modality == "Text", acc_used == 1, !is.na(rt), !is.na(Arousal),
             condition %in% c("Congruent", "Incongruent")) %>%
      group_by(subject, Arousal, condition) %>%
      summarise(mean_rt = mean(rt, na.rm = TRUE), .groups = "drop") %>%
      tidyr::pivot_wider(names_from = condition, values_from = mean_rt) %>%
      mutate(effect = Incongruent - Congruent) %>%
      select(subject, Arousal, effect) %>%
      group_by(subject) %>%
      summarise(levels = sum(!is.na(effect)), .groups = "drop")
    n_obs <- sum(eff_by_aro$levels >= 2)
  }
  ...
  if(p_int < 0.05) {
    cat("<p class='mt-3'><strong>推論：</strong> 喚醒度顯著調節了一致性效應的大小。</p>")
  } else {
    cat("<p class='mt-3'><strong>推論：</strong> 未發現顯著的 Arousal × Condition 交互作用。</p>")
  }
  
  cat(get_perm_text(perm_txt_aro))
...
if (!is.null(beh_trials) && nrow(beh_trials) > 0) {
  ...
  if (nrow(eff_val) > 0) {
    p <- plotly::plot_ly(
      ...
      mode = "lines+markers",
      hovertemplate = paste0(
        "Subject=%{color}<br>Valence=%{x}<br>CE=%{y:.3f}s<extra></extra>"
      )
    ) %>%
      plotly::layout(
        title = "Corresponding Plot: Valence × Condition (CE by Valence; CE = Incongruent - Congruent)",
...
if (!is.null(beh_trials) && nrow(beh_trials) > 0) {
  ...
  if (nrow(eff_aro) > 0) {
    p <- plotly::plot_ly(
      ...
      mode = "lines+markers",
      hovertemplate = paste0(
        "Subject=%{color}<br>Arousal=%{x}<br>CE=%{y:.3f}s<extra></extra>"
      )
    ) %>%
      plotly::layout(
        title = "Corresponding Plot: Arousal × Condition (CE by Arousal; CE = Incongruent - Congruent)",
        xaxis = list(title = "Arousal"),
        yaxis = list(title = "Congruency Effect (s)")
      )
    p
  }
...
<div class="interpretation-card">
<h3 class="card-title">Valence × Arousal × Condition</h3>
```{r txt_3way, results='asis'}
if(!is.null(wc_txt_3way)) {
  p_int <- as.numeric(wc_txt_3way$p)
  stat <- wc_txt_3way$statistic
  n_obs <- if ("n1.y" %in% names(wc_txt_3way)) wc_txt_3way$n1.y else NA
  sig_class <- ifelse(p_int < 0.05, "sig", "nonsig")
  
  if (is.na(n_obs) && !is.null(beh_trials) && nrow(beh_trials) > 0) {
    n_obs <- beh_trials %>%
      filter(Modality == "Text", acc_used == 1, !is.na(rt), !is.na(Valence), !is.na(Arousal)) %>%
  ...
  if(p_int < 0.05) {
    cat("<p class='mt-3'><strong>推論：</strong> 喚醒度顯著調節了「效價對一致性效應的影響」，支持三階交互作用。</p>")
  } else {
    cat("<p class='mt-3'><strong>推論：</strong> 未發現顯著的三階交互作用。</p>")
  }
  
  cat(get_perm_text(perm_txt_3way))
  ...
  if (nrow(txt_subj) > 0) {
    ...
    if (length(panels) >= 2) {
      plotly::subplot(panels, nrows = 1, shareY = TRUE, titleX = TRUE) %>%
        plotly::layout(title = "Corresponding Plot: Valence × Arousal × Condition (Mean ± SE)")
    } else if (length(panels) == 1) {
      panels[[1]] %>% plotly::layout(title = "Corresponding Plot: Valence × Arousal × Condition (Mean ± SE)")
    }
  }
}
```

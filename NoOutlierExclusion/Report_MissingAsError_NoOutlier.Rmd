---
title: "一致性與情緒刺激之行為數據分析（漏答視為錯誤，保留離群值）"
subtitle: "漏答題目轉為錯誤，以完整保留錯誤率的估計（不排除反應時間極端值）"
author: "組員劉鎮臺編寫報告"
...
# rt_max <- 1.5
exclude_ids <- c()
missing_as_error_param <- TRUE
keep_outliers_param <- TRUE
scenario_title <- "漏答視為錯誤（保留離群值）"
...
# 漏答視為錯誤：將 NA 的 acc 轉為 0，保留於錯誤率分析
if (file.exists(data_path_raw)) {
  df_raw_source <- read.csv(data_path_raw, stringsAsFactors = FALSE)
} else {
  stop("找不到原始資料檔案！", data_path_raw)
}

df <- df_raw_source %>%
  # Raw 資料來源時，先排除程序 event 列（避免把開始/結束當成 trial）
  { if ("event" %in% names(.)) dplyr::filter(., is.na(event) | event == "") else . } %>%
  mutate(
    acc = ifelse(is.na(acc), 0, acc),           # 漏答 → 錯誤
  ...
  # RT 範圍限制；保留 NA 以便在 ACC/卡方分析中視為錯誤
  # filter(rt >= rt_min | is.na(rt), rt <= rt_max | is.na(rt)) %>%  # 不排除 RT 離群值
  group_by(subject) %>%
  mutate(
...
# 共用函數：若為 HTML 則輸出 datatable，否則 kable
...
# IAT 樣式 D 分數計算（以 Congruent vs Incongruent 為相容/不相容）
...
- 剔除反應時間缺失或正確率缺失的試驗。
- 限制反應時間在 0.2–1.5 秒之間，以排除極端過快與過慢反應。
- 於受試者內計算 RT z-score，並標記離群值（絕對值 > 2.5）以供後續品質檢查。
...
## 2.4 行為資料快速視覺化（情境對齊；互動式）

```{r behavior_overview_scenario}
if (!exists("data_path_raw") || !file.exists(data_path_raw)) {
  cat("（找不到 analysis_data.csv，無法繪製行為圖。）")
} else {
  raw_all <- read.csv(data_path_raw, stringsAsFactors = FALSE) %>%
    mutate(
  ...
  if (length(exclude_ids) > 0) {
    trials_all <- trials_all %>% filter(!subject %in% exclude_ids)
  }
  ...
  # 套用 outlier / RT 範圍規則（情境）
  trials_used <- trials_all
  if (!keep_outliers_param) {
    if (missing_as_error_param) {
      trials_used <- trials_used %>% filter(is.na(rt) | (rt >= rt_min & rt <= rt_max))
    } else {
      trials_used <- trials_used %>% filter(!is.na(rt), !is.na(acc_used), rt >= rt_min, rt <= rt_max)
    }
  } else {
    if (!missing_as_error_param) {
      trials_used <- trials_used %>% filter(!is.na(rt), !is.na(acc_used))
    }
  }
  ...
  sum_tbl <- tibble(
    ...
    Subjects = n_distinct(trials_all$subject),
    Trials_Stored = nrow(trials_all),
    Trials_Used = nrow(trials_used),
    Keep_Outliers = keep_outliers_param,
    Missing_As_Error = missing_as_error_param
  )
  
  out <- list(
    as_table(sum_tbl, caption = "資料摘要（Stored vs Used；已套用情境規則）"),
    as_table(per_subj, caption = "每位受試者 Stored 試次摘要（應為 120；含練習/正式）"),
    htmltools::HTML("<p><small>說明：本段圖表以 analysis_data.csv 的 trial 列為基礎，並依本情境套用排除受試者／漏答視為錯誤／RT 範圍或保留離群值規則。</small></p>")
  )
  ...
  p_acc <- plotly::plot_ly(
    ...
    plotly::layout(
      title = "Accuracy: Condition × Modality (Missing as Error)",
      barmode = "group",
  ...
  rt_dat <- trials_used %>% filter(!is.na(rt))
  if (nrow(rt_dat) > 0) {
    ...
    make_violin <- function(mod) {
      ...
      plotly::plot_ly(
        ...
        plotly::layout(
          title = paste0("RT 分布（", mod, "；僅非缺失 RT）"),
          xaxis = list(title = "Condition"),
  ...
  # 3) 個體一致性效應（正確試次；Incongruent - Congruent；僅非缺失 RT）
  eff <- trials_used %>%
  ...
  # 5b) 每位受試者：Mean RT（正確）與 Accuracy（雙軸）
  subj_overall <- trials_used %>%
    group_by(subject) %>%
    summarise(
      mean_rt_correct = mean(rt[acc_used == 1], na.rm = TRUE),
  ...
  if (nrow(subj_overall) > 0) {
    p_dual <- plotly::plot_ly(subj_overall, x = ~subject) %>%
      plotly::add_bars(
        y = ~mean_rt_correct,
        ...
        text = ~sprintf("Subject=%s<br>Mean RT(correct)=%.3f s<br>n_correct_rt=%d", subject, mean_rt_correct, n_correct_rt),
        hoverinfo = "text"
      ) %>%
      plotly::add_bars(
        y = ~acc_mean,
    ...
    # 靜態：並排版（避免雙軸難以閱讀）
    png_side <- file.path("..", "Behavior_Figures", scenario_id, "rt_accuracy_side_by_side.png")
    if (file.exists(png_side)) {
      out <- append(out, list(
        htmltools::HTML("<p><small>圖片版（並排；含數值標記）</small></p>"),
        htmltools::tags$img(src = knitr::image_uri(png_side), style = "max-width: 100%; height: auto;")
      ))
    } else {
      out <- append(out, list(htmltools::HTML("<p><small>（尚未找到並排圖片版；請先執行 make_behavior_plots.R 產生 Behavior_Figures/&lt;Scenario&gt;/rt_accuracy_side_by_side.png。）</small></p>")))
    }
  ...
  # 8) Demo 風格：RT 一致性效應（分開：受試者別＋群體平均）
  demo_subj_abs <- file.path("..", "Behavior_Figures", scenario_id, "demo_rt_congruency_subject.png")
  demo_group_abs <- file.path("..", "Behavior_Figures", scenario_id, "demo_rt_congruency_group.png")
  if (file.exists(demo_subj_abs) || file.exists(demo_group_abs)) {
    out <- append(out, list(htmltools::HTML("<h4>Demo 風格：RT 一致性效應（分開）</h4>")))
    if (file.exists(demo_subj_abs)) {
      out <- append(out, list(
        htmltools::HTML("<p><strong>受試者別（Congruent vs Incongruent）</strong></p>"),
        htmltools::tags$img(src = knitr::image_uri(demo_subj_abs), style = "max-width: 100%; height: auto;")
      ))
    }
    if (file.exists(demo_group_abs)) {
      out <- append(out, list(
        htmltools::HTML("<p><strong>群體平均（Congruent vs Incongruent）</strong></p>"),
        htmltools::tags$img(src = knitr::image_uri(demo_group_abs), style = "max-width: 100%; height: auto;")
      ))
    }
  }
...
tryCatch({
  ...
  if (knitr::is_html_output()) ggplotly(p_dist) else p_dist
}, error = function(e) {
  cat("Dashboard 圖形生成失敗：", e$message)
})
```
...
if (has_image_type) {
  p_img_det <- df %>%
    filter(Modality == "Image", !is.na(rt)) %>%
...
if (nrow(subj_cong) > 0) {
  p_cong <- ggplot(subj_cong, aes(x = Condition, y = mean_rt, group = subject)) + 
    ...
    scale_color_manual(values = c("Congruent" = "#00AFBB", "Incongruent" = "#FC4E07")) + 
    theme_pubr() + 
    labs(title = "Congruency Main Effect (RT)", y = "Mean RT (s)")
...
### 4.2.2 分模態 IAT D 分數（Image vs Text）
...
if (nrow(subj_img) > 0) {
  p_img <- ggplot(subj_img, aes(x = Condition, y = mean_rt, color = Image_Type, group = interaction(Image_Type, subject))) + 
    ...
    theme_pubr() + 
    labs(title = "Image Task: Condition × Image_Type", y = "Mean RT (s)")
...
if (nrow(subj_txt) > 0) {
  p_txt <- ggplot(subj_txt, aes(x = Valence, y = mean_rt, color = Condition, group = Condition)) + 
    ...
    facet_wrap(~ Arousal) + 
    theme_pubr() + 
    labs(title = "Text Task: Valence × Condition (Faceted by Arousal)",
         y = "Mean RT (s)")
...
```{r data_clean_dt}
as_table(df, caption = "Cleaned Data (Missing as Error, No Outlier Removal)")
```

## 原始資料 (Raw Data)

```{r data_raw_dt}
if (exists("df_raw_source")) {
  as_table(df_raw_source, caption = "Original Raw Data (before NA imputation)")
} else {
  cat("原始資料檔案未找到或未載入。" )
}
```
...
```{r export_xlsx}
xlsx_name <- "Master_Statistics_MissingAsError.xlsx"

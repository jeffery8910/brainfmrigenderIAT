---
title: "一致性與情緒刺激之行為數據分析（排除 S5 / S10 版本，保留離群值）"
subtitle: "移除受試者 S5、S10 之後的完整分析（不排除反應時間極端值）"
author: "組員劉鎮臺編寫報告"
...
scenario_title <- "排除 S5 / S10（保留離群值）"
...
本研究亦存在若干限制。首先，樣本數相對有限，在推廣至更廣泛族群時需謹慎；未來研究可增加受試者數量，或納入不同文化背景以檢驗結果的普遍性。其次，圖片與文字刺激的情緒強度與社會意涵雖已事先規劃，但仍可能存在個體差異；未來可搭配主觀評價量表或生理指標（例如心跳、皮膚電反應）進行多模態佐證。最後，雖然本報告依照事前計畫採用重複量數 ANOVA 與基礎 GLMM/LMM，但更進階的階層貝氏模型或反應時分布模型（例如 ex-Gaussian）亦值得未來嘗試。

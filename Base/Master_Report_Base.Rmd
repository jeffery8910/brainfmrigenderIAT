---
title: "一致性與情緒刺激之行為數據分析"
subtitle: "依統計分析計畫執行之完整報告"
author: "組員劉鎮臺編寫報告"
...
# Scenario parameters (keep in sync with cleaned data used below)
exclude_ids <- character(0)
missing_as_error_param <- FALSE
keep_outliers_param <- FALSE
scenario_title <- "基礎分析 (Base)"

# Initialize Excel workbook data list
excel_sheets <- list()

# Adjusted paths for rendering from subdirectory (Base/...)
...
if (!exists("data_path_raw") || is.null(data_path_raw) || !file.exists(data_path_raw)) {
  ...
  out <- list(
    ...
    htmltools::HTML("<p><small>說明：本段圖表以 analysis_data.csv 的 trial 列為基礎，並依本情境套用：排除受試者／漏答視為錯誤／RT 範圍或保留離群值規則。</small></p>")
  )
...
if (knitr::is_html_output()) {
  tryCatch(ggplotly(p_dist), error = function(e) p_dist)
} else {
  p_dist
}
```
...
# Save to Excel list (need to convert table/test to data frame)

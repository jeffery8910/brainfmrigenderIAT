---
title: "一致性與情緒刺激之行為數據分析（漏答視為錯誤版本）"
subtitle: "漏答題目轉為錯誤，以完整保留錯誤率的估計"
author: "組員劉鎮臺編寫報告"
...
keep_outliers_param <- FALSE
scenario_title <- "漏答視為錯誤"
...
df <- df_raw_source %>%
  ...
  filter(rt >= rt_min | is.na(rt), rt <= rt_max | is.na(rt)) %>%
  group_by(subject) %>%
  mutate(
...
```{r gpower_congruency, echo=FALSE}
alpha <- 0.05
target_power <- 0.80

if (!exists("diff_vec")) {
  cat("（找不到一致性差值向量 diff_vec，無法估計檢定力／樣本數。）")
} else {
  diffs <- diff_vec
  diffs <- diffs[is.finite(diffs)]

  if (length(diffs) < 2 || sd(diffs) == 0) {
    cat("（差值變異為 0 或樣本數不足，無法估計檢定力／樣本數。）")
  } else {
    ...
    excel_sheets[["GPower_Congruency"]] <- gp_table
    as_table(gp_table, caption = "G*Power 等同設定：一致性主效應（配對 t 檢定）的檢定力與樣本數估計")
    ...
    excel_sheets[["GPower_Reference_N"]] <- gp_ref
    as_table(gp_ref, caption = "參考：不同假設效應量 d_z 下，達目標 power 所需樣本數（配對數）")
  }
}
```
...
```{r data_clean_dt}
as_table(df, caption = "Cleaned Data")
```
